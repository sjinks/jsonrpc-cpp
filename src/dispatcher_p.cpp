...
using shared_lock = std::unique_lock<std::shared_mutex>;
...
void dispatcher_private::add_handler(std::string_view method, dispatcher::handler_t&& handler)
{
    const unique_lock lock(this->m_methods_mutex);
    this->m_methods.try_emplace(std::string(method), std::move(handler));
}
...
dispatcher::handler_t dispatcher_private::get_method(const std::string& method)
{
    const shared_lock lock(this->m_methods_mutex);
...
{
    const auto handler = this->get_method(method);
    this->q_ptr->on_method(method);
    const auto response = handler(params);
